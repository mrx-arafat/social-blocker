/**
 * URL Monitor — detects SPA navigation to Shorts pages on YouTube.
 *
...
import { isShortsUrl } from "@/utils/blockers";

type UrlChangeCallback = (url: string, isShortsPage: boolean) => void;
...
function checkUrl(): void {
  ...
  if (newUrl !== currentUrl) {
    currentUrl = newUrl;
    callback?.(newUrl, isShortsUrl(newUrl));
  }
}
...
export function startUrlMonitor(onUrlChange: UrlChangeCallback): void {
  ...
  // Initial check
  onUrlChange(currentUrl, isShortsUrl(currentUrl));
}
